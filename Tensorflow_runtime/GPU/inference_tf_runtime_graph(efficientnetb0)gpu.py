...
MODEL_PATH = 'Tensorflow_runtime/Models/efficientnetb0_best.h5'
...
# CPU affinity and threading (optional)
...
# === Graph Optimization: XLA JIT Compilation ===
...
# Warm-up
if image_paths:
    warmup_tensor = tf.convert_to_tensor(preprocess_image(image_paths[0]))
    _ = optimized_inference(warmup_tensor)

for img_path, true_class in tqdm(zip(image_paths, image_labels), total=len(image_paths), desc="Processing Images"):
    input_tensor = tf.convert_to_tensor(preprocess_image(img_path))
    times = []
    ...
    for _ in range(num_repeats):
        t0 = time.time()
        output = optimized_inference(input_tensor)
        t1 = time.time()
        times.append(t1 - t0)
...
# === Print Report ===
print(f"\n=== TensorFlow Inference Report ({architecture}) on {device} ===")
